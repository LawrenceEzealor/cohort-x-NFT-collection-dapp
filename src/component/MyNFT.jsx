    ...
    <div className="w-full grid lg:grid-cols-3 md:grid-cols-2 gap-6">
      {isLoading ? (
        ...
        myTokensData.map((x) => (
          <div
            key={x.dna}
            className="w-full flex flex-col gap-2 p-4 border border-blue-500/20 rounded-md"
          >
            ...
            <Text className="block text-base font-serif font-semibold text-pink-950">
            ...
            <Dialog.Root>
              <div className="flex justify-between items-center w-full p-4 border border-blue-500/30 rounded-md">
                <a
                  ...
                  target="_blank"
                  className="text-gray-100 underline font-serif font-bold text-sm "
                  rel="noopener noreferrer"
                >
                  OpenSea
                </a>
                <Dialog.Trigger>
                  <button className="text-gray-100 bg-pink-950 px-4 py-1.5 font-serif font-bold text-sm rounded-md">
                    Transfer
                  </button>
                </Dialog.Trigger>
              </div>

              <Dialog.Content style={{ maxWidth: 40 }}>
                ...
                <Flex gap="3" mt="4" justify="end">
                  ...
                  <Dialog.Close>
                    <button
                      onClick={() => handleTransfer(x.edition)}
