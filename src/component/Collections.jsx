      ...
      {tokensData.length === 0 ? (
        ...
        tokensData.map((x, i) => {
          return (
            <div
              key={x.dna}
              className="w-full flex flex-col p-4 border border-blue-600/20 rounded-md"
            >
              ...
              <Text className="block text-base font-serif font-semibold text-green-500">
                <span className="text-gray-500">Name:</span> {x.name}
              </Text>
                ...
                <Dialog.Root>
                  <Dialog.Trigger>
                    <div className="flex justify-between items-center w-full p-4 border border-blue-500/30 rounded-md">
                      <button className="bg-purple-500 w-full text-sm font-serif font-bold py-2 rounded-md text-gray-100">
                        Mint Nft
                      </button>
                    </div>
                  </Dialog.Trigger>
                  ...
                  <Dialog.Content style={{ maxWidth: 450 }}>
                    ...
                    <Flex gap="3" mt="4" justify="end">
                      ...
                      <Dialog.Close>
                        <button
                          onClick={() => handleMint(x.edition)}
                          className="bg-pink-950 px-6 py-2 rounded-md text-white"
                        >
                          Mint
                        </button>
                      </Dialog.Close>
...
const NFTLink = ({ item }) => {
  return (
    <div
      key={item.index}
      className="flex justify-between items-center w-full p-4 border border-blue-500/30 rounded-md"
    >
      ...
      <a
        ...
        target="_blank"
        className="text-gray-100 bg-blue-500 px-4 py-1.5 font-serif font-bold text-sm rounded-md"
